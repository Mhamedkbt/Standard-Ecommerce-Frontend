            ...
            <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:translate-x-0 md:flex md:flex-col md:static md:inset-0`}>
                ...
                <div className="flex items-center h-16 flex-shrink-0 px-4 bg-gray-800 border-b border-indigo-700">
                    <h1 className="text-2xl font-extrabold text-white">
                        <i className="fas fa-cubes text-indigo-400 mr-2"></i> Admin Panel
                    </h1>
                </div>
                ...
                <div className="flex-1 flex flex-col overflow-y-auto">
                    <nav className="flex-1 px-3 py-4 space-y-1">
                        {navigation.map((item) => (
                            <button
                                ...
                                } group flex items-center px-4 py-2.5 text-base font-semibold rounded-xl w-full text-left transition duration-150 ease-in-out`}
                            >
                                <svg
                                    className={`${
                                        activePage === item.name.toLowerCase() ? 'text-indigo-100' : 'text-gray-400 group-hover:text-gray-300'
                    ...
                    <div className="flex-shrink-0 flex border-t border-gray-700 p-4">
                        <button
                            ...
                            className="flex-shrink-0 w-full group flex items-center px-4 py-2.5 text-base font-semibold rounded-xl text-red-400 bg-gray-800 hover:bg-red-700 hover:text-white transition duration-150 ease-in-out"
                        >
                            <svg
                                className="mr-3 flex-shrink-0 h-6 w-6 text-red-500 group-hover:text-white"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                aria-hidden="true"
